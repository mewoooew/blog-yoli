...
then 
  # 如果存在相似文件名
  if [ -e "./content/zh/posts/$category/"*"_$postfilename.md" ]  
  then 
    # 询问重新建立文件名还是打开已有文件
...
case $isold in
  "e")
    ...
    dirfile="./content/zh/posts/$category/"*"_$postfilename.md"
  ;;
  "d")
    # 根据分类和文件名建立路径
    postfilename=""$today"_draft.md"
  ...
  ;;
  "f")
    postfilename=""$today"_"$postfilename".md"
    # 根据分类和文件名创建新文章
    dirfile="./content/zh/posts/$category/$postfilename"
    hugo new posts/$category/$postfilename
  ;;
